...
- Security Rules（#23）採 Rule Name 分組的專用檢視，參數以橫向欄位呈現並支援拖曳排序。
- 可忽略 `to.member`、`from.member`、`profile-setting.group.member`，讓核對聚焦於其他設定。
- 每條 Rule 最下方會依 A/B 差異生成可在 B 設備執行的 PA CLI；遮罩敏感值不產生命令，僅提示人工設定。
- 新增目前分類的單頁 Excel 匯出，會帶入畫面篩選條件及 Security Rules CLI 補充指令。
